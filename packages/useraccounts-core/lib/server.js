...
AT.prototype._init = function () {
  ...
  if (
    ...
    (!Accounts.oauth || Accounts.oauth.serviceNames().length === 0)
  ) {
    throw Error(
      "AccountsTemplates: You must add at least one account service!",
    );
  }
  ...
  if (!username && !email) {
    throw Error(
      "At least one field out of username and email is strictly required!",
    );
  }
  ...
  if (email) {
    ...
    if (username) {
      ...
      if (username.type !== "text") {
        throw Error(
          "The type of username field should be text when email field is present!",
        );
      }
    } else {
      // email only
      if (!email.required) {
        throw Error(
          "The email field should be required when username is not present!",
        );
      }
    }
  } else {
      ...
      if (attempt.error) {
        ...
        if (reason === "User not found" || reason === "Incorrect password") {
          throw new Meteor.Error(
            403,
            AccountsTemplates.texts.errors.loginForbidden,
          );
        }
      }
  ...
  if (
    ...
    this.options.enforceEmailVerification
  ) {
    Accounts.validateLoginAttempt(function (attempt) {
      if (!attempt.allowed) {
        return false;
      ...
      if (!ok) {
        throw new Meteor.Error(
          401,
          AccountsTemplates.texts.errors.verifyEmailFirst,
        );
      }
  ...
  if (this.options.showReCaptcha) {
    ...
    var settingsSecretKey =
      Meteor.settings.reCaptcha && Meteor.settings.reCaptcha.secretKey;

    if (!atSecretKey && !settingsSecretKey) {
      throw new Meteor.Error(
        401,
        "User Accounts: reCaptcha secret key not found! Please provide it or set showReCaptcha to false.",
      );
    }
  }

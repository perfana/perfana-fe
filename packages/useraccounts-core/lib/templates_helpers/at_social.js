...
AT.prototype.atSocialHelpers = {
  disabled: function () {
    ...
    if (user) {
      var numServices = 0;
      if (user.services) numServices = _.keys(user.services).length; // including "resume"
      if (numServices === 2 && user.services[this._id]) return "disabled";
    }
  },
  name: function () {
    return this._id;
  ...
  iconClass: function () {
    ...
    if (!ic) ic = "fa fa-" + this._id;
    return ic;
  },
  buttonText: function () {
    var service = this;
    var serviceName = this._id;
    if (serviceName === "meteor-developer") serviceName = "meteor";
    serviceName = capitalize(serviceName);
    if (!service.configured)
      return (
        T9n.get(
          AccountsTemplates.texts.socialConfigure,
          (markIfMissing = false),
        ) +
        " " +
        serviceName
    ...
    if (user && showAddRemove) {
      if (user.services && user.services[this._id]) {
        ...
        if (numServices === 2) return serviceName;
        else
          return (
            T9n.get(
              AccountsTemplates.texts.socialRemove,
              (markIfMissing = false),
            ) +
            " " +
            serviceName
        ...
        return (
          T9n.get(AccountsTemplates.texts.socialAdd, (markIfMissing = false)) +
          " " +
          serviceName
        );
    }
    ...
    var state =
      (parentData && parentData.state) || AccountsTemplates.getState();
    var prefix =
      state === "signIn"
    ...
    return (
      prefix +
      " " +
      T9n.get(AccountsTemplates.texts.socialWith, (markIfMissing = false)) +
      " " +
      serviceName
    );
  },
...
AT.prototype.atSocialEvents = {
  "click button": function (event, t) {
    ...
    if (AccountsTemplates.disabled()) return;
    var user = Meteor.user();
    if (user && user.services && user.services[this._id]) {
      ...
      else methodName = "loginWith" + capitalize(serviceName);
      ...
      loginWithService(options, function (err) {
        ...
        } else AccountsTemplates.submitCallback(err, state);
      });
    }
  },
};

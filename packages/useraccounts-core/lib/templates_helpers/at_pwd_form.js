...
AT.prototype.atPwdFormHelpers = {
  ...
  showForgotPasswordLink: function () {
    ...
    var state =
      (parentData && parentData.state) || AccountsTemplates.getState();
    return (
      state === "signIn" && AccountsTemplates.options.showForgotPasswordLink
    );
  },
  ...
  "submit #at-pwd-form": function (event, t) {
    ...
    _.each(AccountsTemplates.getFields(), function (field) {
      ...
      if (preValidation && field.getStatus() !== false) {
        ...
        if (validationErr) {
          if (field.negativeValidation) field.setError(validationErr);
          else {
    ...
    if (state === "signUp") {
      ...
      return Meteor.call("ATCreateUserServer", options, function (error) {
        ...
        AccountsTemplates.submitCallback(error, undefined, function () {
          if (
            AccountsTemplates.options.sendVerificationEmail &&
            AccountsTemplates.options.enforceEmailVerification
          ) {
            AccountsTemplates.submitCallback(error, state, function () {
              AccountsTemplates.state.form.set(
                "result",
                AccountsTemplates.texts.info.signUpVerifyEmail,
              );
      ...
      return Accounts.forgotPassword(
        ...
        function (error) {
          AccountsTemplates.submitCallback(error, state, function () {
            AccountsTemplates.state.form.set(
              "result",
              AccountsTemplates.texts.info.emailSent,
            );
      ...
      return Accounts.resetPassword(paramToken, password, function (error) {
        AccountsTemplates.submitCallback(error, state, function () {
          ...
          if (state === "resetPwd")
            AccountsTemplates.state.form.set(
              "result",
              AccountsTemplates.texts.info.pwdReset,
            ); // Enroll Account
          else
            AccountsTemplates.state.form.set(
              "result",
              AccountsTemplates.texts.info.pwdSet,
            );
      ...
      return Accounts.changePassword(
        ...
        function (error) {
          AccountsTemplates.submitCallback(error, state, function () {
            AccountsTemplates.state.form.set(
              "result",
              AccountsTemplates.texts.info.pwdChanged,
            );
    ...
    if (state === "resendVerificationEmail") {
      return Meteor.call("ATResendVerificationEmail", email, function (error) {
        AccountsTemplates.submitCallback(error, state, function () {
          AccountsTemplates.state.form.set(
            "result",
            AccountsTemplates.texts.info.verificationEmailSent,
          );
          t.$("#at-field-email").val("");

...
AT.prototype.atFormHelpers = {
  ...
  showTitle: function (next_state) {
    ...
    if (Meteor.userId() && state === "signIn") return false;
    return !!AccountsTemplates.texts.title[state];
  },
  showOauthServices: function (next_state) {
    var state = next_state || this.state || AccountsTemplates.getState();
    if (!(state === "signIn" || state === "signUp")) return false;
    var services = AccountsTemplates.oauthServices();
    if (!services.length) return false;
    if (Meteor.userId()) return AccountsTemplates.options.showAddRemoveServices;
    return true;
  },
  showServicesSeparator: function (next_state) {
    ...
    return (
      ...
      !Meteor.userId() &&
      pwdService &&
      AccountsTemplates.oauthServices().length
    );
  },
  ...
  showPwdForm: function (next_state) {
    if (Package["accounts-password"] === undefined) return false;
    var state = next_state || this.state || AccountsTemplates.getState();
  ...
  showSignInLink: function (next_state) {
    if (AccountsTemplates.options.hideSignInLink) return false;
    var state = next_state || this.state || AccountsTemplates.getState();
    if (
  ...
  showSignUpLink: function (next_state) {
    if (AccountsTemplates.options.hideSignUpLink) return false;
    var state = next_state || this.state || AccountsTemplates.getState();
    return (
      ((state === "signIn" && !Meteor.userId()) || state === "forgotPwd") &&
      !AccountsTemplates.options.forbidClientAccountCreation
    );
  },
  ...
  showResendVerificationEmailLink: function () {
    ...
    return (
      (state === "signIn" || state === "forgotPwd") &&
      AccountsTemplates.options.showResendVerificationEmailLink
    );
  },
};

...
AT.prototype.atInputRendered = [
  function () {
    ...
    if (AccountsTemplates.options.focusFirstInput) {
      var firstVisibleInput = _.find(
        AccountsTemplates.getFields(),
        function (f) {
...
AT.prototype.atInputHelpers = {
  ...
  optionalText: function () {
    return (
      "(" +
      T9n.get(AccountsTemplates.texts.optionalField, (markIfMissing = false)) +
      ")"
    );
  },
  templateName: function () {
    if (this.template) return this.template;
    if (this.type === "checkbox") return "atCheckboxInput";
    if (this.type === "select") return "atSelectInput";
    if (this.type === "radio") return "atRadioInput";
    if (this.type === "hidden") return "atHiddenInput";
    return "atTextInput";
  },
  values: function () {
    var id = this._id;
...
AT.prototype.atInputEvents = {
  ...
  "keyup input": function (event, t) {
    ...
    if (!field.continuousValidation) return;
    var parentData = Template.parentData();
    var state =
      (parentData && parentData.state) || AccountsTemplates.getState();
    // No validation during signIn

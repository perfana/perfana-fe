...
FIELD_PAT = {
  ...
  displayName: Match.Optional(
    Match.OneOf(String, Match.Where(_.isFunction), FIELD_SUB_PAT),
  ),
...
AT.prototype.texts = {
  ...
  info: {
    ...
    verificationEmailSent:
      "A new email has been sent to you. If the email doesn't show up in your inbox, be sure to check your spam folder.",
  },
  inputIcons: {
...
AT.prototype.addField = function (field) {
  ...
  if (this._initialized) {
    throw new Error(
      "AccountsTemplates.addField should strictly be called before AccountsTemplates.init!",
    );
  }
...
AT.prototype.addFields = function (fields) {
  ...
  try {
    ...
    throw new Error(
      "field argument should be an array of valid field objects!",
    );
  }
  if (ok) {
    _.map(
...
AT.prototype.configure = function (config) {
  ...
  if (this._initialized) {
    throw new Error(
      "Configuration options must be set before AccountsTemplates.init!",
    );
  }
  ...
  if (reCaptcha) {
    ...
    this.options.reCaptcha = _.defaults(
      reCaptcha,
  ...
  if (config.texts) {
    ...
    if (texts.inputIcons) {
      ...
      this.texts.inputIcons = _.defaults(
        texts.inputIcons,
        this.texts.inputIcons,
      );
    }

    if (texts.socialIcons) {
      // Updates the current socialIcons object
      this.texts.socialIcons = _.defaults(
        texts.socialIcons,
        this.texts.socialIcons,
      );
    }
...
AT.prototype.configureRoute = function (route, options) {
  console.warn(
    "You now need a routing package like useraccounts:iron-routing or useraccounts:flow-routing to be able to configure routes!",
  );
};
...
AT.prototype.removeField = function (fieldId) {
  ...
  if (this._initialized) {
    throw new Error(
      "AccountsTemplates.removeField should strictly be called before AccountsTemplates.init!",
    );
  }

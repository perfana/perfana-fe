...
UniSelectize = function (options, template) {
  ...
  this.sortMethod = _.isUndefined(options.sortMethod)
    ? "label"
    : options.sortMethod;
...
UniSelectize.prototype.itemsAutorun = function () {
  ...
  if (this.placeholder && this.optionsPlaceholder) {
    itemsUnselected.unshift({
      value: "",
      label: _.isString(this.optionsPlaceholder)
...
UniSelectize.prototype.createItem = function () {
  ...
  if (template.uniSelectize.createMethod) {
    Meteor.call(
      template.uniSelectize.createMethod,
      searchText,
      searchText,
...
UniSelectize.prototype.getItemsUnselectedFiltered = function () {
  ...
  return _.filter(items, function (item) {
    if (
      ...
      item.label.search(new RegExp(searchText, "i")) !== -1
    ) {
      return true;
    }
    return false;
...
Template.universeSelectize.events({
  ...
  "keydown input.js-universeSelectizeInput": function (e, template) {
    ...
    switch (e.keyCode) {
      ...
      case 40: // down
        if (
          activeOption < itemsUnselected.length - 1 ||
  ...
  "click .selectize-dropdown-content > div:not(.create)": function (
    e,
    template,
  ) {
    e.preventDefault();

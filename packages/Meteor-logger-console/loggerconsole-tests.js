...
log1.info(
  "From: " +
    (Meteor.isServer ? "[SERVER] " : "[CLIENT] ") +
    "(no data object) Some info string ",
);
log1.debug(
  "From: " +
    (Meteor.isServer ? "[SERVER] " : "[CLIENT] ") +
...
log1.debug(
  "From: " +
    (Meteor.isServer ? "[SERVER] " : "[CLIENT] ") +
    "(no data object) Some debug string ",
);
log1.error(
  "From: " +
    (Meteor.isServer ? "[SERVER] " : "[CLIENT] ") +
...
log1.error(
  "From: " +
    (Meteor.isServer ? "[SERVER] " : "[CLIENT] ") +
    "(no data object) Some error string ",
);
log1.fatal(
  "From: " +
    (Meteor.isServer ? "[SERVER] " : "[CLIENT] ") +
...
log1.fatal(
  "From: " +
    (Meteor.isServer ? "[SERVER] " : "[CLIENT] ") +
    "(no data object) Some fatal string",
);
log1.warn(
  "From: " +
    (Meteor.isServer ? "[SERVER] " : "[CLIENT] ") +
...
log1.warn(
  "From: " +
    (Meteor.isServer ? "[SERVER] " : "[CLIENT] ") +
    "(no data object) Some warn string ",
);
log1.trace(
  "From: " +
    (Meteor.isServer ? "[SERVER] " : "[CLIENT] ") +
...
log1.trace(
  "From: " +
    (Meteor.isServer ? "[SERVER] " : "[CLIENT] ") +
    "(no data object) Some trace string ",
);
log1._("From: " + (Meteor.isServer ? "[SERVER] " : "[CLIENT] ") + "_ ", {
  test: "_",
});
log1._(
  "From: " +
...
Tinytest.add("LoggerMessage Instance", (test) => {
  ...
  test.instanceOf(
    log.fatal(
      'This is message "fatal"',
      { data: 'Sample data "fatal"' },
      'userId "fatal"',
    ),
    LoggerMessage,
  );
  test.instanceOf(
    log.warn(
      'This is message "warn"',
      { data: 'Sample data "warn"' },
      'userId "warn"',
    ),
    LoggerMessage,
  );
  test.instanceOf(
    log.trace(
      'This is message "trace"',
      { data: 'Sample data "trace"' },
      'userId "trace"',
    ),
    LoggerMessage,
  );
  test.instanceOf(
    log._('This is message "_"', { data: 'Sample data "_"' }, 'userId "_"'),
    LoggerMessage,
  );
});
...
Tinytest.add("LoggerMessage#toString", (test) => {
  ...
  test.equal(
    log
      ._('This is message "_"', { data: 'Sample data "_"' }, 'userId "_"')
      .toString(),
...
Tinytest.add("Trace", (test) => {
  if (Meteor.isServer) {
    test.isTrue(
      _.has(log.trace(602, { data: 602 }, 602).details, "stackTrace"),
    );
